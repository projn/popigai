...
function config()
{
    ...
    src='\#node.name: node-1'
    dst='node.name: '${ELASTICSEARCH_NODE_NAME}
    sed -i "s#$src#$dst#g" ${config_path}

    src='\#path.data: /path/to/data'
    dst='path.data: '${SOFTWARE_DATA_PATH}
    sed -i "s#$src#$dst#g" ${config_path}

    src='\#path.logs: /path/to/logs'
    dst='path.logs: '${SOFTWARE_LOG_PATH}
    sed -i "s#$src#$dst#g" ${config_path}

    src='\#network.host: 192.168.0.1'
    dst='network.host: '${ELASTICSEARCH_HOST}
    sed -i "s#$src#$dst#g" ${config_path}

    src='\#http.port: 9200'
    dst='http.port: '${ELASTICSEARCH_PORT}
    sed -i "s#$src#$dst#g" ${config_path}

    src='\#discovery.zen.ping.unicast.hosts: \["host1", "host2"\]'
    dst='discovery.zen.ping.unicast.hosts: \['${ELASTICSEARCH_CLUSTER_HOST_LIST}'\]'
    sed -i "s#$src#$dst#g" ${config_path}

    src='\#discovery.zen.minimum_master_nodes: '
    dst='discovery.zen.minimum_master_nodes: 2'
    sed -i "s#$src#$dst#g" ${config_path}
